    ...
    <x-tasks.task-layout>
        ...
        <div class="mb-4">
            ...
            <div class="bg-yellow-100 p-4 rounded shadow-sm">
                <p class="text-yellow-900 leading-relaxed">{{ $task->body }}</p>
            </div>
        </div>
        ...
        @if($task->path)
            <div class="mb-4">
                ...
                <a href="{{ asset('storage/' . $task->path) }}" target="_blank" class="text-blue-600 hover:underline">
                    📁 Скачать файл
                </a>
            </div>
        @endif
        ...
        <div class="flex justify-between mt-6">
            <a href="{{ route('tasks.index') }}"
               class="bg-yellow-200 text-yellow-900 px-4 py-2 rounded hover:bg-yellow-300 shadow-sm">
                ⬅ Назад
            </a>
            <div class="flex gap-2">
                <a href="{{ route('tasks.edit', $task) }}"
                   class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 shadow-sm">
                    ✏ Редактировать
                </a>
                <form action="{{ route('tasks.destroy', $task) }}" method="POST"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 shadow-sm">
                        🗑 Удалить
                    </button>
                </form>
            </div>
