    ...
    <x-tasks.task-layout>
        ...
        <x-forms.form method="POST" action="{{ route('tasks.update', $task) }}" enctype="multipart/form-data">
            ...
            <div class="flex gap-4 mt-4">
                <button type="submit" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600 shadow-sm">
                    💾 Сохранить
                </button>
                <a href="{{ route('tasks.show', $task) }}"
                   class="bg-yellow-200 text-yellow-900 px-4 py-2 rounded hover:bg-yellow-300 shadow-sm">⬅ Назад</a>
            </div>
        </x-forms.form>
    </x-tasks.task-layout>
</x-app-layout>
